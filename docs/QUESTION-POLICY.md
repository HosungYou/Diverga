...
> response-only advisory questions belong in the assistant response or explicit
> audit output, not raw hook context; durable Researcher Checkpoints are reserved
...
- 연구 질문 또는 범위 변경
- 이론 프레임 또는 construct map 변경
- 측정, 코딩, 추출 기준 변경
- 방법 설계 또는 분석 전략 변경

반대로 reversible draft, formatting, low-risk engineering task는 가정을
명시하고 진행해도 된다.
...
- never inject raw advisory-question lists into Codex hook context
...
- research question/scope, theory frame, measurement/coding standard, method
  design, analysis strategy를 바꾸려 할 때
...
- 이전 요청과 새 요청이 충돌하거나, 인간 지식/AI 추론/project state가
  서로 다른 방향을 가리킬 때
...
- `LongTable surfaced response-only advisory questions...` 같은 내부 hook
  하네싱 문구를 사용자-facing context로 흘리는 것
